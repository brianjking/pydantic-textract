...
def process_image_with_textract(image_bytes):
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error processing document with Textract: {e}")
...
def call_llama_index_to_process_data(extracted_text, schema_cls, schema_name):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error generating schema with LlamaIndex: {e}")

@app.post("/process_document")
async def process_document(selected_schema_name: str, file: UploadFile = File(...), authenticated: bool = Depends(authenticate)):
    ...
    if extracted_text:
        ...
        if structured_data:
            return JSONResponse(content=structured_data.dict())
        else:
